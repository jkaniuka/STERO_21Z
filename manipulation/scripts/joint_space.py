...
def T_B_Wr_aroud_goal(point, velma):
    T_B_Jar_angle=math.atan2(point.y(), point.x())
    
    rotation_angle=T_B_Jar_angle
    print(rotation_angle)
    R=0.0 # R = 0.3
    angle_step=math.pi/32 # angle_step 
    steps = int(round(math.pi/angle_step))
    T_B_Wr = []
...
# Generate possible destinations (for jar object)
def T_B_Wr_aroud_goalall(point, velma):
    ...
    # for step in range(steps):
    #     angle = angle_step * step
    x = point.x()# + R * math.cos(rotation_angle)
    y = point.y()# + R * math.sin(rotation_angle)
    z = point.z() 
    roll = pitch = 0.0
    yaw = rotation_angle# correct_angle(math.pi + rotation_angle)
    rotation = PyKDL.Rotation.RPY(roll, pitch, yaw)
    vector = PyKDL.Vector(x, y, z)
    frame =  PyKDL.Frame(rotation, vector)# * PyKDL.Frame(PyKDL.Rotation.RPY(0,math.pi/2,0))
    # T_B_Wr.append(frame*velma.getTf('Gr', 'Wr'))
    T_B_Wr.append(frame)
    print("CEL", T_B_Wr[0]) 
    print("CEL.M", T_B_Wr[0].M)
    print("CEL.p", T_B_Wr[0].p)
    return T_B_Wr
